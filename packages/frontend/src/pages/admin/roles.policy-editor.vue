...
<div class="_gaps_s">
	...
	<XFolder v-if="matchQuery([i18n.ts._role._options.onlyFollowersNote, 'onlyFollowersNote'])" v-model:policyMeta="policyMetaModel.onlyFollowersNote" :isBaseRole="isBaseRole" :readonly="readonly">
		<template #label>{{ i18n.ts._role._options.onlyFollowersNote }}</template>
		<template #valueText>{{ valuesModel.onlyFollowersNote ? i18n.ts.yes : i18n.ts.no }}</template>
		<template #default="{ disabled }">
			<MkSwitch v-model="valuesModel.onlyFollowersNote" :disabled="disabled">
				<template #label>{{ i18n.ts.enable }}</template>
			</MkSwitch>
		</template>
	</XFolder>
