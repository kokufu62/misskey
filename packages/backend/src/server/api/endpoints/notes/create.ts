...
export default class extends Endpoint<typeof meta, typeof paramDef> { // eslint-disable-line import/no-default-export
	constructor(
		...
		super(meta, paramDef, async (ps, me) => {
			// createdAtの形式チェック (HEADの変更)
			if (ps.createdAt != null && isNaN(Date.parse(ps.createdAt))) {
				throw new ApiError(meta.errors.invalidCreatedAt);
			}

			// upstreamのロジックに従い、fetchAndCreateを使用してノートを作成
			try {
				const note = await this.noteCreateService.fetchAndCreate(me, {
					createdAt: ps.createdAt ? new Date(ps.createdAt) : new Date(), // createdAtの値を反映
					fileIds: ps.fileIds ?? ps.mediaIds ?? [], // upstreamのファイルID渡し方を採用
